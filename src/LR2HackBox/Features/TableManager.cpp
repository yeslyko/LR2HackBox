...
//TableManager::Table::Table(std::string name, std::string symbol) : name(std::move(name)), symbol(std::move(symbol)), path(this->name + '/') {}
...
void TableManager::Table::WriteFile() {
    ...
    for (auto& entry : entries) {
        nlohmann::ordered_json json = entry.originalJson;
        json["level"] = entry.level;
        json["md5"] = entry.md5;
        if (json.contains("title")) json["title"] = entry.title;
        else if (!entry.title.empty()) json.push_back({ "title", entry.title });
        if (json.contains("artist")) json["artist"] = entry.artist;
        else if (!entry.artist.empty()) json.push_back({ "artist", entry.artist });
        data.push_back(json);
    }
...
bool TableManager::CreateTable(std::string name, std::string symbol) {
    if (!std::ranges::contains(mTables, name, &Table::name) && !std::filesystem::exists(mOutputPaths[mOutputIdx]/name)) {
        nlohmann::ordered_json originalJson = {
            {"name", name},
            {"symbol", symbol},
            {"data_url", "./data.json"}
        };
        mTables.emplace_back(name, symbol, mOutputPaths[mOutputIdx]/name, "data.json", originalJson);
...
bool TableManager::RenameTable(std::string name, std::string newName) {
    if (auto table = std::ranges::find(mTables, name, &Table::name); table != mTables.end() && !std::ranges::contains(mTables, newName, &Table::name)) {
        table->name = newName;
        int nameIdx = 1;
        auto tableName = std::ranges::find(mTableNames, name);
        *tableName = newName;
    }
    return false;
}
    ...
    for (auto& dir : std::filesystem::directory_iterator(selectedPath)) {
        ...
        for (auto& file : std::filesystem::directory_iterator(dir.path())) {
            ...
            try {
                ...
                std::ifstream dataFile(dir.path()/table.dataPath);
                if (dataFile.good()) {
                    try {
                ...
                std::sort(selectedTable->entries.begin(), selectedTable->entries.end(), [sort_specs](const Entry& left, const Entry& right) {
                    for (int i = 0; i < sort_specs->SpecsCount; i++) {
                        ...
                        if (spec.ColumnIndex == 1) {
                            ...
                            int rightLevel{};
                            {
                                auto [ptr, ec] = std::from_chars(left.level.data(), left.level.data() + left.level.size(), leftLevel);
                                if (ec != std::errc()) stringMode = true;
                            }
                            {
                                auto [ptr, ec] = std::from_chars(right.level.data(), right.level.data() + right.level.size(), rightLevel);
                                if (ec != std::errc()) stringMode = true;
                            }
                            if (stringMode) {
